...
<div class="h-bar logo-bg">
	<input name="path" value="<?=((PATH)?$path['dirname']:'patterns/json')?>">
	<input name="pname" placeholder="pattern name" value="<?=((PATH)?$path['filename']:'')?>" data-translate="placeholder" pattern="[a-zA-Z0-9_-]+">
	<div class="toolbar">
...
(function(form){
	...
	form.save.onclick=function(event){
		...
		var named = form.pname.value.trim();
		if(path && named) XHR.push({
			addressee:"/patterns/actions/save-pattern/"+named+"?path="+path,
